...
mapper = torch.load(f"{prefix}/models/mapper_new").cuda()
affine_transform = np.load(f"{prefix}/models/affine_trasnform_new.npz")
A =  torch.Tensor(affine_transform["A"]).cuda()
B =  torch.Tensor(affine_transform["B"]).cuda()

def get_stdev(batch_number):
    ...
    # # s = 1
    # # b = 40
    stdev =  a * (np.exp(np.log(s/a) * n_buckets/b) - 1)
    return max(stdev,0)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    logname = f"stealing{args.datasetsteal}_{args.num_queries}_{args.losstype}_defence_{args.usedefence}_sybil_{args.sybil}_{mod}.log"
    logging.basicConfig(
    ...
    if args.resume:
        checkloc = f"{pathpre}/SimCLR/SimSiam/checkpoint_{args.datasetsteal}_{args.losstype}_{args.num_queries}_{mod}.pth.tar"
...
def train(train_loader, model, victim_model, criterion, optimizer, epoch, args):
    ...
    for i, (images, classes) in enumerate(train_loader):
        ...
        if args.sybil  == "True":
            batches_per_attacker = int((args.num_queries/args.batch_size)//2)
        else:
             batches_per_attacker = int((args.num_queries/args.batch_size))+1


        #skip batches used for remapping
        if args.sybil  == "True" and (batches_per_attacker+20)>i and i>batches_per_attacker:
            continue
        ...
        if args.sybil == "True" and i<20:
            ...
            if args.usedefence:
                ...
                g_cuda.manual_seed(i+100000)
                stdev_value = get_stdev(i%batches_per_attacker)
...
def save_checkpoint(state, is_best, args):
    if is_best:
        torch.save(state,
                   f"{pathpre}/SimCLR/SimSiam/checkpoint_{args.datasetsteal}_{args.losstype}_{args.num_queries}_defence_{args.usedefence}_sybil_{args.sybil}_{mod}.pth.tar")
