...
parser.add_argument('--beta', default=80, type=float, metavar='M',
                    help='Beta')
parser.add_argument('--lam', default=0.000001, type=float, metavar='M',
                    help='Lambda')
...
parser.add_argument('--usedefence', default='False', type=str,
                    help='Use defence by noising')
parser.add_argument('--sybil', default='False', type=str,
                    help='Use adaptive attack from 2 accounts')
...
prefix = scratch + '/user/DatasetInferenceForSelfSupervisedModels/'
pathpre = scratch + f"/user/DatasetInferenceForSelfSupervisedModels/checkpoint"


best_acc1 = 0


def get_stdev(batch_number, lam, alpha, beta):
    buckets = [0, 5, 10, 13, 15, 18, 20, 22, 24, 25, 26, 28, 29, 30, 31, 33, 34, 35, 36, 37, 38, 39, 41, 41, 42, 43, 43, 44, 44, 45, 46, 46, 47, 48, 48, 49, 49, 49, 50, 50, 50, 51, 51, 51, 52, 52, 52, 53, 53, 53, 54, 54, 54, 54, 54, 55, 55, 55, 55, 56, 56, 57, 57, 57, 58, 58, 58, 58, 59, 59, 59, 59, 60, 60, 60, 60, 61, 61, 61, 62, 62, 62, 63, 63, 63, 63, 64, 64, 64, 65, 65, 65, 65, 65, 65, 66, 66, 66, 66, 66, 66, 67, 67, 67, 67, 67, 67, 68, 68, 68, 68, 68, 68, 68, 68, 68, 69, 69, 69, 69, 69, 69, 69, 69, 69, 70, 70, 70, 70, 70, 70, 70, 70, 70, 70, 71, 71, 71, 71, 71, 71, 71, 71, 71, 71, 71, 71, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 72, 73, 73, 73, 73, 73, 73, 73, 73, 73, 73, 73, 73, 73, 74, 74, 74, 74, 74, 74, 74, 74, 74, 74, 74, 74, 74, 74, 75, 75, 75, 75, 75, 75, 75, 75, 75, 75]
    buckets_256 = np.repeat(buckets,2)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    logname = f"stealing{args.datasetsteal}_{args.num_queries}_{args.losstype}_defence_{args.usedefence}_sybil_{args.sybil}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils.log"
    logging.basicConfig(
    ...
    checkpoint = torch.load(
        prefix + "/models/checkpoint_0099-batch256.pth.tar",
        map_location="cpu")
    ...
    if args.resume:
        checkloc = f"{pathpre}/SimCLR/SimSiam/checkpoint_{args.datasetsteal}_{args.losstype}_{args.num_queries}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils.pth.tar"
    ...
    # if args.sybil:
    #     mapper = SimpleNet(2048,2048,2048)
    #     mapper = torch.load("/home/jk/diffusers/DatasetInferenceForSelfSupervisedModels/models/mapper")
    #     affine_transform = np.load("/home/jk/diffusers/DatasetInferenceForSelfSupervisedModels/models/affine_trasnform.npz")
    ...
    if args.datasetsteal == 'imagenet':
        ...
        train_dataset_raw = datasets.CIFAR10(
            root=prefix + "/datasets/cifar10", train=True,
            download=True, transform=transform_train)

        test_dataset = datasets.CIFAR10(
            root=prefix + "/datasets/cifar10", train=False,
            download=True, transform=transform_train)
        ...
        train_dataset = datasets.CIFAR100(
            root=f'{prefix}/data/cifar100', train=True,
            download=True, transform=transform_train)

        test_dataset = datasets.CIFAR100(
            root=f'{prefix}/data/cifar100', train=False,
        ...
        train_dataset = datasets.SVHN(
            root=f'{prefix}/data/SVHN', split='extra',
...
def train(train_loader, model, victim_model, criterion, optimizer, epoch, args):
    ...
    if args.sybil == "True":
        mapper_1 = SimpleNet(2048,2048,2048)
        mapper_1 = torch.load(f"/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/models/mapper/mapper_{args.num_queries_mapping}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils_1").cuda()
        affine_transform_1 = np.load(f"/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/models/affine/affine_transform_{args.num_queries_mapping}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils_1.npz")
        A_1 =  torch.Tensor(affine_transform_1["A"]).cuda()
        B_1 =  torch.Tensor(affine_transform_1["B"]).cuda()

        mapper_2 = SimpleNet(2048,2048,2048)
        mapper_2 = torch.load(f"/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/models/mapper/mapper_{args.num_queries_mapping}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils_2").cuda()
        affine_transform_2 = np.load(f"/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/models/affine/affine_transform_{args.num_queries_mapping}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils_2.npz")
        A_2 =  torch.Tensor(affine_transform_2["A"]).cuda()
        B_2 =  torch.Tensor(affine_transform_2["B"]).cuda()

        mapper_3 = SimpleNet(2048,2048,2048)
        mapper_3 = torch.load(f"/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/models/mapper/mapper_{args.num_queries_mapping}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils_3").cuda()
        affine_transform_3 = np.load(f"/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/models/affine/affine_transform_{args.num_queries_mapping}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils_3.npz")
        A_3 =  torch.Tensor(affine_transform_3["A"]).cuda()
        B_3 =  torch.Tensor(affine_transform_3["B"]).cuda()
    ...
    for i, (images, classes) in enumerate(train_loader):
        ...
        if args.sybil  == "True":
            batches_per_attacker = int(np.ceil((args.num_queries/args.batch_size)//4))
        else:
             batches_per_attacker = int((args.num_queries/args.batch_size))+1

        #skip batches used for remapping
        if args.sybil  == "True" and (batches_per_attacker+batches_for_mapping )>i and i>=batches_per_attacker:
            continue
        elif args.sybil  == "True" and (2*batches_per_attacker+batches_for_mapping )>i and i>=2*batches_per_attacker:
            continue
        elif args.sybil  == "True" and (3*batches_per_attacker+batches_for_mapping )>i and i>=3*batches_per_attacker:
            continue
        ...
        if args.usedefence  == "True":
            ...
            if args.sybil  == "True" :
                stdev_value = get_stdev(i%batches_per_attacker, args.lam, args.alpha, args.beta)
            else:
                stdev_value = get_stdev(i, args.lam, args.alpha, args.beta)
            #print(i,stdev_value)
            #print(torch.normal(0,stdev_value,size=victim_features.shape, generator=g_cuda))
            victim_features = victim_features + torch.normal(0,stdev_value,size=victim_features.shape, generator=g_cuda).cuda()

        if args.sybil  == "True" and 2*batches_per_attacker>i and i>=batches_per_attacker:
            victim_features = torch.matmul(victim_features,A_1) + B_1
            victim_features = victim_features*1000
            victim_features = mapper_1(victim_features)
            victim_features = victim_features/1000

        if args.sybil  == "True" and 3*batches_per_attacker>i and i>=2*batches_per_attacker:
            victim_features = torch.matmul(victim_features,A_2) + B_2
            victim_features = victim_features*1000
            victim_features = mapper_2(victim_features)
            victim_features = victim_features/1000

        if args.sybil  == "True" and 4*batches_per_attacker>i and i>=3*batches_per_attacker:
            victim_features = torch.matmul(victim_features,A_3) + B_3
            victim_features = victim_features*1000
            victim_features = mapper_3(victim_features)
            victim_features = victim_features/1000
...
def save_checkpoint(state, is_best, args):
    if is_best:
        torch.save(state,
                   f"{pathpre}/SimCLR/SimSiam/checkpoint_{args.datasetsteal}_{args.losstype}_{args.num_queries}_defence_{args.usedefence}_sybil_{args.sybil}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils.pth.tar")
