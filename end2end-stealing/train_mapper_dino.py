...
def main(args):
    ...
    imagenet_ds =  np.load("/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/victim_features_50k_dino.npz")["arr_0"].reshape(-1,1536)
    noised_imagenet = noise_dataset(imagenet_ds, 10)
    
    imagenet_ds =  np.load("/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/victim_features_50k_dino.npz")["arr_0"].reshape(-1,1536)
    noised_imagenet_2 = noise_dataset(imagenet_ds, 10)

    imagenet_embeddings = noised_imagenet[:N_queries]
    ...
    imagenet_embeddings_2 = noised_imagenet_2[:N_queries] 
    ...
    transformed_encodings = transformed_encodings.reshape(-1,1536).type(torch.float32)
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < val_min_loss:
           ...
           torch.save(mapper, f"/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/models/mapper/mapper_{args.num_queries_mapping}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_DINO")     

    #=======================
    np.savez(f"/net/tscratch/people/anon/user/DatasetInferenceForSelfSupervisedModels/models/affine/affine_transform_{args.num_queries_mapping}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_DINO.npz", A=A, B=B)
