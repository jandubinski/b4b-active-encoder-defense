...
parser.add_argument('--mod', default='False', type=str,
                    help='model')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.modeltype == "stolen":
        ...
        logname = f"testing{args.datasetsteal}{args.num_queries}{args.losstype}{args.dataset}_defence_{args.usedefence}_sybil_{args.sybil}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils.log"
        logging.basicConfig(
            filename=os.path.join(log_dir, logname),
            level=logging.DEBUG)
    else:
        logging.basicConfig(
            filename=f"{pathpre}/SimCLR/SimVIC/testing{args.dataset}.log",
            level=logging.DEBUG)
    ...
    if args.modeltype == "stolen":
        checkpoint = torch.load(
            f"{pathpre}/SimCLR/SimSiam/checkpoint_imagenet_infonce_{args.num_queries}_defence_{args.usedefence}_sybil_{args.sybil}_alpha{args.alpha}_beta{args.beta}_lambda{args.lam}_4sybils.pth.tar",
            map_location="cpu")
        state_dict = checkpoint['state_dict']
        print("state_dict")
        ...
        for k in list(state_dict.keys()):
            if k.startswith('module.backbone.'):
                ...
                if k.startswith('backbone.') and not k.startswith(
                        'backbone.fc'):
                    # remove prefix
    ...
    if args.dataset == 'imagenet':
        ...
        train_dataset = datasets.CIFAR10(
            root=f'{prefix}/data/cifar10', train=True,
        ...
        test_dataset = datasets.CIFAR10(
            root=f'{prefix}/data/cifar10', train=False,
        ...
        svhn_path = f"{prefix}/datasets/svhn"
        ...
        test_dataset = datasets.SVHN(
            svhn_path, split='test', download=True,
        ...
        stl_path = f"{prefix}/datasets/stl10"
        ...
        fmnist_path = f"{prefix}/datasets/fmnist"
